"""Add dewpoint_avg, dewpoint_min, dewpoint_max to aggregate views.
...
def upgrade() -> None:
    ...
    # Recreate hourly with dewpoint columns
    op.execute("""
    ...
    # Recreate daily with dewpoint columns
    op.execute("""
    ...
    # Recreate monthly with dewpoint columns
    op.execute("""
    ...
    # Re-enable real-time aggregation (007 set this, but WITH NO DATA defaults
    # to materialized_only=true, so we must re-apply after recreating)
    for view in _VIEWS:
        op.execute(
...
def downgrade() -> None:
    # Reverse: drop and recreate without dewpoint (back to 004/007 state)
    op.execute("SELECT remove_continuous_aggregate_policy('observations_monthly', if_not_exists => true)")
    op.execute("DROP MATERIALIZED VIEW IF EXISTS observations_monthly CASCADE")
    op.execute("SELECT remove_continuous_aggregate_policy('observations_daily', if_not_exists => true)")
    op.execute("DROP MATERIALIZED VIEW IF EXISTS observations_daily CASCADE")
    ...
    # Re-enable real-time aggregation (restoring 007 state)
    for view in _VIEWS:
        op.execute(
            f"ALTER MATERIALIZED VIEW {view} SET (timescaledb.materialized_only = false)"
        )
